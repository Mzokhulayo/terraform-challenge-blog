...
A workspace is a named state file within the same backend and the same configuration directory. When you create a workspace and apply, Terraform writes a separate state file under a workspace-specific path in S3:
...
## What File Layouts Do

File layouts use completely separate directories per environment, each with its own `backend.tf` pointing to a unique state file key:
